...
class ContentCache:
    ...
    def check_videos_exist(self, channels: List[str], date: Optional[str] = None) -> Tuple[bool, List[Dict]]:
        ...
        # Scan for existing video files
        for video_file in date_path.glob("*.mp4"):
            # Extract channel info from filename
            filename = video_file.name
            
            # Map different channel name variations
            channel_mappings = {
                'Money Purse': 'moneypurse',
                'DAY TRADER': 'daytradertelugu',
                'moneypurse': 'moneypurse',
                'daytradertelugu': 'daytradertelugu'
            }
            
            for channel_variant, canonical_name in channel_mappings.items():
                if channel_variant in filename:
                    channels_found.add(canonical_name)
    ...
    def check_transcripts_exist(self, video_info: List[Dict], date: Optional[str] = None) -> Tuple[bool, List[Dict]]:
        ...
        for video in video_info:
            ...
            # Look for transcript file patterns
            transcript_patterns = [
                f"{channel}_{video_id}.json",
                f"{channel}_*.json"
            ]
            
            transcript_found = False
            for pattern in transcript_patterns:
                transcript_files = list(date_path.glob(pattern))
                if transcript_files:
                    transcript_file = transcript_files[0]  # Take first match
